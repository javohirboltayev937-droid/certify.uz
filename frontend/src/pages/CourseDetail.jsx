...
import { PlayCircleIcon, LockClosedIcon, DocumentTextIcon, ClockIcon, UserGroupIcon } from '@heroicons/react/24/outline'
...
export default function CourseDetail() {
  ...
  const LESSON_ICONS = { video: '▶️', text: '📄', quiz: '❓', audio: '🔊' }
      ...
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 text-white py-12 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-gray-400 text-sm mb-4">
            ...
            <span>/</span>
            <span style={{ color: course.subject.color }}>{course.subject.name}</span>
          </div>
          <h1 className="text-3xl font-extrabold mb-3">{course.title}</h1>
          <p className="text-gray-300 mb-6">{course.description}</p>
          <div className="flex flex-wrap gap-4 text-sm text-gray-400">
            <span className="flex items-center gap-1"><PlayCircleIcon className="w-4 h-4" /> {course.total_lessons} dars</span>
            <span className="flex items-center gap-1"><ClockIcon className="w-4 h-4" /> {course.total_duration} daqiqa</span>
            <span className="flex items-center gap-1"><UserGroupIcon className="w-4 h-4" /> {course.enrolled_count} o'quvchi</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-2">
              {course.lessons.map((lesson, i) => {
                ...
                return (
                  <div key={lesson.id}
                    className={`card p-4 flex items-center gap-4 ${locked ? 'opacity-60' : ''}`}>
                    ...
                    {locked ? (
                      ...
                      <span className="badge bg-green-100 text-green-700 text-xs">Bepul</span>
                    ) : null}
                  </div>
                )
              })}
                ...
                <div className="space-y-2">
                  {course.materials.map(m => (
                    <div key={m.id} className="card p-3 flex items-center gap-3">
                      ...
                      <span className="badge bg-gray-100 text-gray-600 text-xs ml-auto">{m.material_type.toUpperCase()}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div>
            <div className="card p-6 sticky top-20">
              ...
              {course.is_enrolled ? (
                <button className="btn-success w-full mb-3">✓ Yozilgansiz</button>
              ) : (
                <button
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-600">
                ...
                <div className="flex justify-between">
                  <span>Fan:</span>
                  <span className="font-medium" style={{ color: course.subject.color }}>{course.subject.name}</span>
                </div>
              </div>
            </div>
          </div>
