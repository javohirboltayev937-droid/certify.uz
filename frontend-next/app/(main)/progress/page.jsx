...
import { TrophyIcon, ChartBarIcon, FireIcon, BookOpenIcon, CheckCircleIcon, XCircleIcon } from '@heroicons/react/24/outline'
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold mb-6 flex items-center gap-2">
          <ChartBarIcon className="w-7 h-7 text-primary-600" /> Mening Progressim
        </h1>

        {/* Stats */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Jami testlar',   value: data?.stats?.total || 0,                    icon: BookOpenIcon,  bg: 'bg-blue-50',   text: 'text-blue-600' },
            { label: "O'rtacha ball",  value: `${Math.round(data?.stats?.avg_score || 0)}%`,   icon: ChartBarIcon, bg: 'bg-purple-50', text: 'text-purple-600' },
            { label: 'Eng yuqori',     value: `${Math.round(data?.stats?.best_score || 0)}%`, icon: TrophyIcon,   bg: 'bg-amber-50',  text: 'text-amber-600' },
            { label: 'Streak',         value: `${data?.streak?.current_streak || 0} kun`,    icon: FireIcon,     bg: 'bg-orange-50', text: 'text-orange-600' },
              ...
              {data.by_type.map(({ exam_type, count, avg }) => (
                <div key={exam_type} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-primary-500 rounded-full transition-all duration-500" style={{ width: `${avg || 0}%` }} />
                  </div>
        ...
        {data?.achievements?.length > 0 && (
          <div className="mb-8">
            <h2 className="font-bold mb-4 flex items-center gap-2"><TrophyIcon className="w-5 h-5 text-amber-500" /> Yutuqlar</h2>
