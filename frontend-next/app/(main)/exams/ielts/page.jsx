...
const SECTIONS = [
  { id: 'listening', label: 'Listening', icon: SpeakerWaveIcon,   desc: "40 savol · 30 daqiqa + 10 daqiqa ko'chirish vaqti", color: 'bg-blue-50 border-blue-200' },
  { id: 'reading',   label: 'Reading',   icon: BookOpenIcon,       desc: '40 savol · 60 daqiqa. Academic va General',         color: 'bg-green-50 border-green-200' },
  { id: 'writing',   label: 'Writing',   icon: PencilSquareIcon,   desc: '2 ta task · 60 daqiqa. Graph/Essay',               color: 'bg-purple-50 border-purple-200' },
  { id: 'speaking',  label: 'Speaking',  icon: MicrophoneIcon,     desc: '3 qism · 11-14 daqiqa. Ekzaminator bilan',         color: 'bg-orange-50 border-orange-200' },
]

const BAND_SCORES = [
  { band: '9.0',     level: 'Expert',    color: 'bg-purple-600', usage: 'Magistratura (top universitetlar)' },
  { band: '8.0-8.5', level: 'Very Good', color: 'bg-blue-600',   usage: 'Magistratura (xalqaro)' },
  { band: '7.0-7.5', level: 'Good',      color: 'bg-green-600',  usage: 'Bakalavr (xalqaro)' },
  { band: '6.0-6.5', level: 'Competent', color: 'bg-yellow-600', usage: "Ish va ta'lim (Evropa/Avstraliya)" },
  { band: '5.0-5.5', level: 'Modest',    color: 'bg-orange-600', usage: 'Asosiy kommunikatsiya' },
  { band: '4.0-4.5', level: 'Limited',   color: 'bg-red-600',    usage: 'Cheklangan foydalanish' },
]
      ...
      <div className="bg-gradient-to-br from-blue-700 to-blue-900 text-white py-14 px-4">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-4xl font-extrabold mb-2 flex items-center gap-3">
            <GlobeAltIcon className="w-10 h-10" /> IELTS Tayyorgarlik
          </h1>
          <p className="text-blue-200 text-lg mb-6">
            International English Language Testing System — Band 4.0 dan 9.0 gacha
          </p>
          <div className="flex flex-wrap gap-3">
            <Link href="/dtm" className="bg-white text-blue-800 font-semibold px-6 py-3 rounded-xl hover:bg-blue-50 inline-flex items-center gap-2">
              Mock test ishlash →
            </Link>
            <Link href="/courses?subject=english" className="border-2 border-white/30 text-white font-semibold px-6 py-3 rounded-xl hover:bg-white/10 inline-flex items-center gap-2">
              Kurslar ko'rish
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
          {SECTIONS.map(s => (
            <div key={s.id} className={`card p-5 border ${s.color}`}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3" style={{ background: 'rgba(0,0,0,0.05)' }}>
            ...
            <tbody>
              {BAND_SCORES.map(({ band, level, color, usage }, i) => (
                <tr key={band} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-500">{usage}</td>
                </tr>
              ))}
            </tbody>
          </table>
